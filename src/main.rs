...
impl HelloWorld for HelloServer {
    fn hello_message(&self, _m: grpc::RequestOptions, args: HelloMessageArgs) -> grpc::SingleResponse<HelloMessageReply> {
        let mut reply = HelloMessageReply::new();
        let name: String = if args.get_name().is_empty() { "World".to_string() } else { args.get_name().to_string() };
